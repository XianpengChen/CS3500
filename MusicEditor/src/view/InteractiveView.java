...
/**
 * Created by Xianpeng on 4/7/2017.
 */
public class InteractiveView extends JFrame implements IView {
    ...
    public InteractiveView(MusicModel model, Sequencer sqcer) {
        this.model = model;
        this.sqcer = sqcer;
        js = null;
        jc = null;
    ...
    public void initialize() throws InvalidMidiDataException, MidiUnavailableException {
        ...
        js.addMouseListener(new MouseListentoJS());
        js.setSize(1500, 700);
    ...
    private class MouseListentoJS implements MouseListener {
        Date pressTime;
        ...
        public void mouseReleased(MouseEvent e) {
            long length = new Date().getTime() - pressTime.getTime();
            int duration = (int) (length / 1000);
            int x = e.getX() + js.getHorizontalScrollBar().getValue();
            int y = e.getY() + js.getVerticalScrollBar().getValue();
            int atVolume = (x - 50) / 20;
            int pitch = start + (y - 80) / 20;

            model.map().get(pitch).put(atVolume, new Beat(1, pitch, 85, 1, duration));
            while (duration > 0) {
                model.map().get(pitch).put(atVolume + duration, new Beat(1, pitch,
                        85, 0, 0));
                duration--;
            }


            Track noe = sqcer.getSequence().getTracks()[0];
            MidiMessage star = null;
            try {
                star = new ShortMessage(ShortMessage.NOTE_ON, 1, pitch, 85);
            } catch (InvalidMidiDataException e1) {
                e1.printStackTrace();
            }
            MidiMessage sto = null;
            try {
                sto = new ShortMessage(ShortMessage.NOTE_OFF, 1, pitch, 85);
            } catch (InvalidMidiDataException e1) {
                e1.printStackTrace();
            }
            MidiEvent sta = new MidiEvent(star, atVolume);
            MidiEvent so = new MidiEvent(sto, atVolume + duration);
            noe.add(sta);
            noe.add(so);
            jc.repaint();
